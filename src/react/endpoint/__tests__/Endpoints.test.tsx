...
import { mockOffsetSize, renderWithCustomRoute, renderWithRouterMatch, TEST_API_BASE_URL } from '../../utils/testUtil';
...
describe('Endpoints', () => {
  const selectors = {
    createDataServiceButton: () => screen.getByRole('button', { name: /Create Data Service/ }),
  };
  beforeAll(() => {
  ...
  it('should render the table with the correct columns', async () => {
    ...
    //E
    await waitForElementToBeRemoved(() => screen.getByText('Loading Data Services...'));
    //V
    expect(screen.getByRole('grid')).toBeInTheDocument();
    const sortedEndpoints = ENDPOINTS.sort((a, b) => a.hostname.localeCompare(b.hostname));
    screen.getAllByRole('row').forEach((row, rowIndex) => {
      if (rowIndex === 0) return; //ignore header
      within(row)
        .getAllByRole('gridcell')
        .forEach((cell, index) => {
          if (index === 0) expect(cell).toHaveTextContent(sortedEndpoints[rowIndex - 1].hostname);
          if (index === 1) expect(cell).toHaveTextContent(sortedEndpoints[rowIndex - 1].locationName);
        });
    });
  ...
  describe('Empty State', () => {
    beforeEach(() => {
      server.use(
        rest.get(`${TEST_API_BASE_URL}/api/v1/config/overlay/view/${INSTANCE_ID}`, (_req, res, ctx) =>
          res(
            ctx.json({
    ...
    it('should render Empty State when there are no data services', async () => {
      renderWithRouterMatch(<Endpoints />);
      await waitForElementToBeRemoved(() => screen.getByText('Loading Data Services...'));
      expect(screen.getByText(/A list of Data Services will appear here./i)).toBeInTheDocument();
    });
    it('should redirect to the create data service page when the create data service button is clicked', async () => {
      renderWithCustomRoute(
        <Routes>
          ...
          <Route path="/create-dataservice" element={<div>Create Data Service</div>}></Route>
        </Routes>,
        '/',
      );
      await waitForElementToBeRemoved(() => screen.getByText('Loading Data Services...'));
