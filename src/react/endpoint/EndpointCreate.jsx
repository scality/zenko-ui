...
const schema = Joi.object({
    hostname: Joi.string().label('Host Name').required().min(3).max(63),
        ...
        <F.Form ref={formRef}>
            ...
            <F.Fieldset>
                ...
                <F.Input
                    ...
                    autoComplete='off' />
                <F.ErrorInput id='error-name' hasError={errors.hostname}> {errors.hostname?.message} </F.ErrorInput>
            </F.Fieldset>
            <F.Fieldset>
                <F.Label tooltipMessages={['Cannot be modified after creation']}>
                ...
                <Controller
                    ...
                    render={({ onChange, value: locationName }) => {
                        return <F.Select
                            ...
                            value={locationName}
                        >
                            {Object.values(locations).map((location: any, i) => <F.Select.Option key={i} value={location.name}>{renderLocation(location)}</F.Select.Option>)}
                        </F.Select>;
                    }}
                />
            </F.Fieldset>
            <F.Footer>
                ...
                <F.FooterButtons>
                    ...
                    <Button disabled={loading} id='create-endpoint-btn' variant="primary" onClick={handleSubmit(onSubmit)} label='Create'/>
                </F.FooterButtons>
            </F.Footer>
        </F.Form>
    </FormContainer>;
