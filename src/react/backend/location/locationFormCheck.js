...
function _getNFSTestOptions(version) {
    if (version === 'v3') {
        return { conflictMap: v3ConflictOpts, optionsMap: NFS_V3_OPTIONS };
    }
    if (version === 'v4') {
...
function _checkNFSDetails(details) {
    ...
    if (!host) {
        return { disable: true, errorMessage: '' }; // return no error, incomplete form
    }

    if (!path) {
        return { disable: true, errorMessage: 'Error: Invalid path' };
    }
...
function _checkAdvancedOptions(options) {
    ...
    if (!sizeLimitGB) {
        return { disable: true, errorMessage: '' };
    }
    const val = parseInt(sizeLimitGB, 10);
    if (val <= 0 || isNaN(val)) {
        return { disable: true, errorMessage: 'Error: Size limit must be greater than 0.' };
    }
    return null;
}
...
function locationFormCheck(location) {
    ...
    return locationError || optionsError || { disable: false, errorMessage: '' };
}
