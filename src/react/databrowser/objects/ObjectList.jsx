...
export default function ObjectList({ objects, bucketName, prefixWithSlash, toggled, listType, bucketInfo }: Props){
    ...
    return <L.ListSection id='object-list'>
        <T.HeaderContainer>
            ...
            <T.ButtonContainer>
                <T.ExtraButton id='object-list-upload-button' label='Upload' icon={<i className="fas fa-upload" />} variant='secondary' onClick={() => dispatch(openObjectUploadModal())}/>
                <T.ExtraButton id='object-list-create-folder-button' label='Folder' icon={<i className="fas fa-plus" />} variant='secondary' onClick={() => dispatch(openFolderCreateModal())}/>
                <T.ExtraButton id='object-list-delete-button' label='Delete' icon={<i className="fas fa-trash" />} disabled={isToggledEmpty} variant='danger' onClick={() => dispatch(openObjectDeleteModal())}/>
                <Toggle
