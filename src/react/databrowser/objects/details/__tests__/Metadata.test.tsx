...
describe('Metadata', () => {
  ...
  it('should add new key/value metadata and should trigger function if save button is pressed', () => {
    ...
    optionsComponents.forEach((optionComponent, index) => {
      expect(optionComponent.find('.sc-select__option').first().text()).toBe(
        optionLabels[index],
      );
    });
    // select a key
    ...
    firstItem.find('input.metadata-input-value').simulate('change', {
      target: {
        value: 'value1',
      },
    });
  ...
  it('should render SelectBox with key/value pass in props', () => {
    ...
    // check if item row values are rendered
    const firstItem = items.first();
    expect(firstItem.find('SelectBox[name="mdKeyType"]').prop('value')).toBe(
      'CacheControl',
    );
    expect(
      firstItem.find('SelectBox[name="mdKeyType"]').prop('isDisabled'),
    ).toBe(false);
    expect(firstItem.find('input.metadata-input-value').prop('value')).toBe(
      'no-cache',
    );
    expect(firstItem.find('input.metadata-input-value').prop('disabled')).toBe(
      false,
    );
    ...
    expect(component.find('Button#metadata-button-save').prop('disabled')).toBe(
      false,
    );
  });
  ...
  it('should delete key/value if remove button is pressed', () => {
    ...
    expect(secondItem.find('SelectBox[name="mdKeyType"]').prop('value')).toBe(
      'x-amz-meta',
    );
    expect(
      secondItem.find('input.metadata-input-extra-key').prop('value'),
    ).toBe('cache-type');
    expect(secondItem.find('input.metadata-input-value').prop('value')).toBe(
      '1',
    );
    ...
    expect(firstItem.find('input.metadata-input-extra-key').prop('value')).toBe(
      'cache-type',
    );
    expect(firstItem.find('input.metadata-input-value').prop('value')).toBe(
      '1',
