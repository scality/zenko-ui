// @flow
import type { CloseEndpointDeleteDialogAction, OpenEndpointDeleteDialogAction, ThunkStatePromisedAction } from '../../types/actions';
...
export function openEndpointDeleteDialog(hostname: Hostname): OpenEndpointDeleteDialogAction {
    return {
...
export function deleteEndpoint(hostname: Hostname): ThunkStatePromisedAction {
    return (dispatch, getState) => {
        ...
        return managementClient.deleteConfigurationOverlayEndpoint(params)
            .then(() => dispatch(updateConfiguration()))
            .then(() => dispatch(waitForRunningConfigurationVersionUpdate()))
            .catch(error => dispatch(handleClientError(error)))
