...
describe('PauseAndResume', () => {
  ...
  const server = setupServer(
    ...
    rest.post(`${TEST_API_BASE_URL}/_/backbeat/api/ingestion/pause/someLocation`, (req, res, ctx) =>
      res(ctx.status(200)),
    ),
  );
  ...
  it('should render the component with pause label when ingestion is enabled', async () => {
    server.use(
      rest.get(`${TEST_API_BASE_URL}/api/v1/instance/${INSTANCE_ID}/status`, (req, res, ctx) =>
        res(
          ctx.json({
            metrics: {
              ['ingest-schedule']: {
