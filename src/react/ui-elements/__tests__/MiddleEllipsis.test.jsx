...
describe('MiddleEllipsis', () => {
    const container = { offsetWidth: 50 };
    const sentence = 'My long long long long sentence';
    let txtNode = document.createTextNode(sentence);
    ...
    it('ellipseNode with ellipsisText should display correctly', () => {
        txtNode.textContent = sentence;
        const ellipsisText = '&&&&';
        const isEllipsized = ellipseNode(container, txtNode, sentence, ellipsisText);
        expect(isEllipsized).toBeTruthy();
    ...
    it('ellipseNode with trailingCharCount should display correctly', () => {
        ...
        const isEllipsized = ellipseNode(container, txtNode, sentence, ellipsisText, trailingCharCount);
        expect(isEllipsized).toBeTruthy();

        const ellipsizedText = txtNode.textContent;
        const lengthAfterEllipsis = ellipsizedText.substr(ellipsizedText.indexOf(ellipsisText) + ellipsisText.length).length;
