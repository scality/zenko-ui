...
import {
  mockOffsetSize,
  TEST_API_BASE_URL, Wrapper as wrapper,
} from '../../utils/test';
import {
  render,
  screen,
...
const WithFormProvider = ({children}) => {
  ...
  const childrenWithProps = React.Children.map(children, child => {
    return(
      <>
        <div data-testid="form-expiration">{isValid ? 'form-valid' : 'form-invalid'}</div>
        {child}
      </>
    );
  });
...
describe('ExpirationForm', () => {
  it('should render a form for expiration workflow', async () => {
    try {
        render(<WithFormProvider><ExpirationForm bucketList={S3BucketList} locations={locations}/></WithFormProvider>, {
        wrapper
      });
      ...
      expect(screen.getByText('Remove expired Delete markers')).toBeInTheDocument();
      expect(screen.getByText('Expire incomplete Multipart uploads')).toBeInTheDocument();
      ...
      expect(sourceBucket.lastChild?.textContent).toBe( "bucket1 (us-east-1 / Local Filesystem)");

      const expireCurrentToggleState = screen.getByTestId('toggle-action-expire-current-version');
      ...
      const expirePreviousToggleState = screen.getByTestId('toggle-action-expire-previous-version');
      const expirePrevious = expirePreviousToggleState.querySelector(
        'input[placeholder="previousVersionDelayDaysToggle"]',
      );

      const removeExpiredToggleState = screen.getByTestId('toggle-action-remove-expired-markers');
      const removeExpired = removeExpiredToggleState.querySelector(
        'input[placeholder="expireDeleteMarkersTrigger"]',
      );

      const expireIncompleteMultipartToggleState = screen.getByTestId('toggle-action-expire-incomplete-multipart');
      const expireIncompleteMultipart = expireIncompleteMultipartToggleState.querySelector(
      ...
      expect(sourceBucket.lastChild?.textContent).toBe( "bucket2 (us-east-1 / Local Filesystem)");
