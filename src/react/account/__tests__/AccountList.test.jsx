...
describe('AccountList', () => {
  it('should render AccountList component with homer row selected', () => {
    ...
    const { component } = reduxMount(
      <AccountList accountList={[account1, account2, account3]} />,
    );
    ...
    expect(firstRowColumns[1]).toEqual(
      formatSimpleDate(new Date(account1.createDate)),
    );
    ...
    expect(secondRowColumns[1]).toEqual(
      formatSimpleDate(new Date(account2.createDate)),
    );

    const thirdRow = rows.last();
    expect(thirdRow.prop('isSelected')).toEqual(true);
    const thirdRowColumns = rows
      .last()
      .find(T.Cell)
      .map(column => column.text());
    expect(thirdRowColumns.length).toEqual(2);
    expect(thirdRowColumns[0]).toEqual(account3.userName);
    expect(thirdRowColumns[1]).toEqual(
      formatSimpleDate(new Date(account3.createDate)),
    );
  });
});
