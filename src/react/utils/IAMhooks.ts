...
>(
  ...
  }, [reactQueryOptions.onUnmountOrSettled]);
  ...
  const entities = data?.pages?.flatMap((page) => getEntitiesFromResult(page));
  ...
  useMemo(() => {
    ...
    entities,
    error,
    preventNextPagesRetrieval,
    reactQueryOptions.onPageSuccess,
    reactQueryOptions.onUnmountOrSettled,
  ]);
