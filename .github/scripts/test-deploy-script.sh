...
# Clean up config files before Test 9
sudo rm -f /usr/share/nginx/html/.well-known/configs/*.json

echo "=== Test 9: /data path rewriting ==="
# Test /data path proxy with correct path rewriting
sudo tee /usr/share/nginx/html/.well-known/configs/test9.json > /dev/null << 'EOF'
{
  "spec": {
    "selfConfiguration": {
      "proxy": {
        "data-api": {
          "source": "/data/api",
          "target": "http://artesca-data-management-orbit-api.zenko.svc.cluster.local:5001"
        },
        "data-admin": {
          "source": "/data/admin",
...
# Run the actual deploy script
output=$(sudo DEPLOY_SCRIPT_TEST_MODE="$DEPLOY_SCRIPT_TEST_MODE" bash deploy-script.sh 2>&1)
if echo "$output" | grep -E "(Adding proxy|Processing proxy config)" > /dev/null; then
  echo "✅ Test 9 passed: Script processes /data proxy configurations"
else
  echo "❌ Test 9 failed: Script should process /data proxy configurations"
  echo "Deploy script output:"
  echo "$output"
  exit 1
fi

# Verify /data proxy with path rewriting is present
if grep -q "location /data/api" /etc/nginx/conf.d/default.conf; then
  echo "✅ Test 9 passed: /data/api location correctly inserted"
else
  echo "❌ Test 9 failed: /data/api location not found"
  echo "Generated config:"
  cat /etc/nginx/conf.d/default.conf
  exit 1
fi

# Verify the proxy_pass has the correct path rewriting (should end with /api/)
if grep -A2 "location /data/api" /etc/nginx/conf.d/default.conf | grep -q "proxy_pass.*:5001/api/"; then
  echo "✅ Test 9 passed: /data/api proxy_pass has correct path rewriting"
else
  echo "❌ Test 9 failed: /data/api proxy_pass should rewrite to /api/"
  echo "Expected: proxy_pass http://artesca-data-management-orbit-api.zenko.svc.cluster.local:5001/api/"
  echo "Actual config:"
  grep -A2 "location /data/api" /etc/nginx/conf.d/default.conf
  exit 1
fi

# Verify /data/admin proxy with path rewriting
if grep -A2 "location /data/admin" /etc/nginx/conf.d/default.conf | grep -q "proxy_pass.*:8080/admin/"; then
  echo "✅ Test 9 passed: /data/admin proxy_pass has correct path rewriting"
else
  echo "❌ Test 9 failed: /data/admin proxy_pass should rewrite to /admin/"
  echo "Expected: proxy_pass http://admin-service:8080/admin/"
  echo "Actual config:"
  grep -A2 "location /data/admin" /etc/nginx/conf.d/default.conf
  exit 1
fi
